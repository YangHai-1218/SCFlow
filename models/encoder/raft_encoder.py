...
from ..backbone.resnet import BasicBlock, Bottleneck, ResLayer
...
class RAFTEncoder(BaseModule):
    ...
    _arch_settings = {
        ...
        'Large': (BasicBlock, (2, 2))
    }

    _stem_channels = {'Basic': 64, 'Small': 32, 'Large': 64}

    _base_channels = {'Basic': (64, 96, 128), 'Small': (8, 16, 24), 'Large':(64, 96)}

    _strides = {'Basic': (1, 2, 2), 'Small': (1, 2, 2), 'Large': (1, 2)}

    _dilations = {'Basic': (1, 1, 1), 'Small': (1, 1, 1), 'Large':(1, 1)}
    ...
    def __init__(self,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            self.inplanes = planes * 4 if net_type == 'Small' else planes
            layer_name = f'res_layer{i+1}'
            self.add_module(layer_name, res_layer)
            self.res_layers.append(layer_name)

            last_channel = (
                self.base_channels[-1] * 4
                if net_type == 'Small' else self.base_channels[-1])
    ...
    def forward(self, x: torch.Tensor, return_middle_result=False) -> torch.Tensor:
        """Forward function.
